...
function setup() {
  ...
  cols = width/cSize
  rows = height/cSize
  for (var y=height/9;y<height/1.5;y+=H){
    for (var x=150;x<250;x+=H){
    ...
    for (var i=0;i<cols;i++){
      ...
      for (var j=0;j<rows;j++){
        ...
        if (j>0 && i > 0){
          ...
          if (useLerp){
            var aVal = samples[I][J]
            var bVal = samples[I + 1][J]
            var cVal = samples[I + 1][J + 1]
            var dVal = samples[I][J + 1]

            var amt = (1 - aVal) / (bVal - aVal)
          ...
          var state = cornersBinaryToDecimal(
            corners[I][J],
            corners[I + 1][J],
            corners[I + 1][J + 1],
            corners[I][J + 1],
...
function keyPressed(){
  if (keyCode == 32){
    for (var y=mouseY-H*2;y<mouseY+H*2;y+=H){
      for (var x=mouseX-H*2;x<mouseX+H*2;x+=H){
